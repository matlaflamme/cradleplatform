...
public class PatientController {
	...
	public Patient createPatient(Authentication auth, @RequestBody Patient patient) throws Exception {
		var authorizer = authorizerFactory.construct(auth);
		if (!authorizer.canCreatePatient()) {
			throw new AccessDeniedException("Create patient: permission denied");
		}
	...
	public Reading createReading(Authentication auth, @RequestBody Reading reading) throws Exception {
		...
		requestAccess(auth, reading.getPatientId());
	 ...
	 */
	private void requestAccess(Authentication auth, @NotNull String id) throws AccessDeniedException {
		var authorizer = authorizerFactory.construct(auth);
		if (!authorizer.canAccessPatient(id)) {
			var errMsg = String.format("Access denied: Patient %s", id);
